...
static func transfer_item_to_position(
  ...
  if source is InventoryContainer:
    ...
    original_container = source
    print("Removing from player body")
  ...
  if not added:
    ...
    elif original_container is PlayerBody:
...
static func _is_equipment_compatible(item: InventoryItem, target: PlayerBody) -> bool:
  var slot_name = _infer_slot(item)
  if slot_name == "":
  ...
  print("Slot %s not found in player body" % slot_name)
  return false
