...
@export var backpack: Backpack = null  # Optional backpack in utility slot
...
var scurrent_houlder_x: float = 0.0
...
func _input(event):
    if Input.is_action_just_pressed("open_inventory"):
        # Show inventory with backpack (if equipped)
        var utility = player_body.get_equipped("utility")
        var backpack = null
        if utility: backpack = utility[0].content as Backpack
        inventory_ui.open_inventory(self, backpack)
        Input.set_mouse_mode(Input.MOUSE_MODE_CONFINED)
    elif Input.is_action_pressed("ui_cancel"):
        inventory_ui.hide()
    elif event is InputEventMouseMotion and not inventory_ui.visible:
        Input.set_mouse_mode(Input.MOUSE_MODE_CAPTURED)
