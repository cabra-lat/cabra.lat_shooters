...
@export var world_item_config: WorldItemConfig
...
func _ready():
    ...
    if not world_item_config:
        world_item_config = WorldItemConfig.new()
...
func _setup_collision():
    if area.get_child(0) and inventory_item:
        ...
        # Set collision layers/masks from config
        if world_item_config:
            area.collision_layer = world_item_config.collision_layer
            area.collision_mask = world_item_config.collision_mask
...
func _setup_highlight():
    if highlight_area and world_item_config and world_item_config.enable_highlight:
        highlight_area.monitoring = true
        highlight_area.monitorable = false

        var highlight_shape = SphereShape3D.new()
        highlight_shape.radius = world_item_config.highlight_radius
...
func pick_up(player: PlayerController) -> bool:
    ...
    if not container:
        if world_item_config and world_item_config.debug_mode:
            print("No suitable container found for item")
        return false
    ...
    var success = Inventory.transfer_item(null, container, inventory_item)
    if success:
        item_picked_up.emit(self, player)
        if world_item_config and world_item_config.debug_mode:
            print("World item picked up: %s" % inventory_item.content.name)
        queue_free()
...
func set_highlighted(highlight: bool):
    ...
    if highlight and world_item_config:
...
func _apply_rarity_material(instance: Node3D):
    if not inventory_item or not world_item_config:
        return

    var rarity = _get_item_rarity()
    if rarity != "common" and world_item_config.rarity_materials.has(rarity):
        var material = world_item_config.rarity_materials[rarity]
...
func _find_suitable_container(player: PlayerController) -> Resource:
    ...
    if not equipped_backpack.is_empty():
        ...
        if backpack_item.content is Backpack:
            ...
            if backpack.can_accept_item(inventory_item):
                return backpack
    ...
    if Inventory._is_compatible_with_target(inventory_item, player.equipment):
...
func _on_body_entered(body: Node3D):
    if body is PlayerController:
        ...
        if world_item_config and world_item_config.auto_pickup:
            pick_up(player)
...
func _process(delta: float):
    ...
    # Bobbing animation
    if world_item_config and world_item_config.enable_bobbing:
        bob_time += delta
